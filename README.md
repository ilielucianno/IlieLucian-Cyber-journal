# 🛡️ My Cybersecurity Ideas & Learning Journey

Welcome to my personal cybersecurity repository! Here I document ideas, questions, and discoveries from my learning journey.

---

## 📋 What's inside

| File | Description |
|------|-------------|
| [colour-auth-idea.md](colour-auth-idea.md) | My color-based authentication concept |
| [aes256-future-security.md](aes256-future-security.md) | Deep analysis of AES-256 security |
| [benny-social-network-idea.md](benny-social-network-idea.md) | Complete Benny platform concept - social network + cryptocurrency |

---

## 🎯 Why this repository

I believe the best way to learn cybersecurity is to:
- **Question everything** - even established standards
- **Document my thinking** - so I can revisit and refine ideas
- **Share openly** - because feedback makes ideas better

This is where I dump my cybersecurity thoughts, questions, and "what if" scenarios.

---

## ✅ Completed Certifications & Learning Paths

| Platform | Course | Status |
|----------|--------|--------|
| TryHackMe | Pre Security | ✅ Completed |
| TryHackMe | Security 101 | ✅ Completed |
| TryHackMe | SEC0 | ✅ Completed |
| TryHackMe | SOC Level 1 (SEC1) | ✅ Completed |

---

## 🔄 Currently Pursuing

| Platform | Course | Status |
|----------|--------|--------|
| CompTIA | Security+ | 🔄 In progress |
| CompTIA | Network+ | 🔄 In progress |

---

## 🔜 Coming next

- Zero Trust implementation for small office
- Cloud security (Azure) notes
- More authentication method ideas
- CompTIA Security+ exam (planned 2026)

---

## 🧠 Current ideas being explored

- ✅ Color-based 2FA (first draft)
- ✅ Deep analysis of AES-256 security
- ✅ Complete Benny platform concept - social network + cryptocurrency
- 🔄 More ideas coming as I learn...

---

## 🧠 About Me

I've been passionate about technology since 1995, when I started with HC2000, QBasic, and Turbo Pascal. 
Now I'm channeling that experience into cybersecurity, with a focus on creative problem-solving and 
deep technical understanding.

**Current role:** Technical Department Manager (10+ years IT infrastructure, networking, hardware)

**Certifications:** TryHackMe SOC Level 1

**Goals:** Security+, Network+, Cloud Security (Azure)

---

## 🔗 Connect with me

- GitHub: [@ilielucianno](https://github.com/ilielucianno)
- TryHackMe: ilielucianno
- LinkedIn: [Ilie Lucian](https://www.linkedin.com/in/ilie-lucian-)
